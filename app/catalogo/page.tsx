...
export default function CatalogPage() {
  ...
  const { cart, cartCount, cartTotal, addToCart: ctxAddToCart, removeFromCart, updateQuantity, clearCart, isAnimating } = useCart();
        ...
        <main className="flex-1 p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
            {products.filter((p) => p.visible).map((p) => {
              ...
              const displayImage = generalImages.length > 0 ? generalImages[0] : p.imageUrl ? { id: "legacy", url: p.imageUrl } : null;
              ...
              >
                {displayImage ? (
                  <img src={displayImage.url} alt={p.name} className="w-full h-32 sm:h-40 object-cover" />
                ) : (
